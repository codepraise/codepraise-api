...
namespace :worker do
  namespace :run do
    ...
    task :development => :config do
      sh 'RACK_ENV=development bundle exec shoryuken -r ./workers/appraisal_worker.rb -C ./workers/shoryuken_dev.yml'
    end

    desc 'Run the background cloning worker in testing mode'
    task :test => :config do
      sh 'RACK_ENV=test bundle exec shoryuken -r ./workers/appraisal_worker.rb -C ./workers/shoryuken_test.yml'
    end
